/* =======================================================================
   smooth-gwihon.js — 귀혼 제작 과정 모달 "전용" 부드러운 스크롤(경량 커스텀)
   -----------------------------------------------------------------------
   · 귀혼 본문은 모달 내부 커스텀 스크롤러(#process-gwihon .process__content)가
     스크롤되는 구조. Lenis 는 content 를 transform 하는 방식이라 이 다중 자식
     + ScrollTrigger 커스텀 스크롤러 구조와 충돌 → 네이티브 scrollTop 을 매 프레임
   ...
   · scroll-behavior 는 CSS(#process-gwihon .process__content)에서 auto 로 덮어
     이중 스무딩을 막는다.
   · 가장자리(맨 위/아래)에서 더 밀면 네이티브에 양보 → 오버스크롤 종료 제스처 유지.
   · prefers-reduced-motion → 비활성(네이티브 스크롤).
   · window.__processLenis.gwihon.scrollTo(top,opt) 노출 → process.js 트랙 클릭이 사용.
======================================================================= */
(() => {
  ...
  /* 루프 구동 — gsap.ticker 우선(안정적), 없으면 rAF */
  ...
  const onWheel = (e) => {
    ...
    const max = maxScroll();
    /* 외부 스크롤 후에도 자연스럽게 이어지도록 현재 위치 동기화 */
    if (Math.abs(scroller.scrollTop - current) > 1) current = target = scroller.scrollTop;
  ...
  /* 트랙 클릭/리셋용 부드러운 이동 — process.js 가 호출 */
  ...
  window.__processLenis.gwihon = { scrollTo };
